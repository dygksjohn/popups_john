...
fake_pymilvus.DataType = types.SimpleNamespace(INT64="INT64", FLOAT_VECTOR="FLOAT_VECTOR", VARCHAR="VARCHAR")
...
from pupoo_ai.app.api.routers.moderation import moderate  # noqa: E402
...
class ModerationRouterTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_timeout_is_blocked(self):
        ...
        with patch("pupoo_ai.app.api.routers.moderation.asyncio.wait_for", side_effect=fake_wait_for):
    ...
    async def test_success_response_contains_result_and_action(self):
        with patch(
            "pupoo_ai.app.api.routers.moderation.moderate_with_rag",
            return_value=("PASS", 0.12, "정상입니다.", "rag_watsonx", None, None),
        ):
            response = await moderate(ModerateRequest(content="정상 문장", board_type="POST"))
